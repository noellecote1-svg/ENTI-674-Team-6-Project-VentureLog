...
export default function MetricDetail() {
  ...
  const chartData = [...data.values].sort((a, b) => new Date(a.recordedDate).getTime() - new Date(b.recordedDate).getTime()).slice(-12).map(v => ({
        ...
        <CardContent className="px-2 sm:px-6">
          <div className="h-[250px] sm:h-[300px] w-full">
            {chartData.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-muted-foreground">No data points logged yet.</div>
            )}
          </div>
        </CardContent>
      </Card>
            ...
            <table className="w-full text-sm min-w-[300px]">
              ...
              <tbody className="divide-y">
                ...
                {data.values.length === 0 && (
                  <tr><td colSpan={3} className="p-8 text-center text-muted-foreground">No values logged.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="space-y-4">
          ...
          {data.linkedDecisions.length > 0 ? (
                    ...
                    <Link href={`/decisions/${decision.id}`}>
                      <div className="cursor-pointer hover:text-primary transition-colors" data-testid={`link-decision-${decision.id}`}>
                        <div className="flex items-center justify-between mb-2">
                          <span className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-medium ${decision.status === 'open' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-700'}`}>
            ...
            <div className="p-8 text-center border border-dashed rounded-lg text-muted-foreground bg-transparent">
              No decisions linked to this metric yet.
            </div>
          )}
        </div>
      </div>
