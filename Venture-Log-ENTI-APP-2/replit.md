...
- Pages: Home dashboard (`/`), Journal list (`/journal`), Journal editor (`/journal/new`, `/journal/:id/edit`), Metrics (`/metrics`, `/metrics/:id`), Decision Log (`/decisions`, `/decisions/:id`), AI Coach (`/ai-coach`), Investor Update (`/investor-update`)
...
- **Phase 3 (AI Coach)**: COMPLETE — GPT-5.2 powered chat, persistent conversations in DB (`conversations` + `messages` tables), context-aware system prompt (journal entries + metrics + decisions), SSE streaming, conversation list sidebar, delete conversations
- **Phase 4 (Decision Log)**: COMPLETE — full CRUD, archive/close flows, comment threads
- **Phase 5 (Investor Update)**: COMPLETE — period selector, company/founder name, streaming GPT-5.2 generation pulling metrics with MoM change, decisions, journal highlights; copy-to-clipboard
- **Phase 6 (Auth/Billing)**: NOT STARTED — do not begin until explicitly asked

### AI Integration
- **Provider**: OpenAI via Replit AI Integrations (no user API key required)
- **Model**: gpt-5.2 for both AI Coach and Investor Update
- **DB tables**: `conversations` (id, title, createdAt), `messages` (id, conversationId, role, content, createdAt)
- **Coach system prompt**: dynamically includes last 8 journal entries, active metrics with latest values, open decisions
- **Investor Update**: pulls metrics with YYYY-MM period filtering, MoM % change, closed decisions, journal entries from that period
